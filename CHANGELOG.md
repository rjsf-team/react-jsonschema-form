...
- Fixed an `ErrorSchema` a field raises through `onChange`, on a path that already carried a validator error, being lost as soon as the form derived state again, so a controlled form flickered the error once per keystroke; the stored error list now carries the raise too, so the `ErrorList` says what the field does, and a raise at the root, which replaces every displayed error, no longer replaces the stored validator result, which used to drop every other field's error for good ([#5306](https://github.com/rjsf-team/react-jsonschema-form/pull/5306))
