...
- Upgraded TypeScript to 7.0.2. The root `typecheck` runs in 5 s instead of 22 s; `@rjsf/docs`'s editor-only tsconfig no longer extends `@tsconfig/docusaurus`, which sets `baseUrl`, an option TypeScript 7 removed (Docusaurus's own `@docusaurus/tsconfig` still sets it too)
