...
- Fixed `liveValidate: 'onBlur'` validating whenever the form derived state from new props, rather than only when a field was left, so replacing the form data from outside showed the errors before any blur. An `'onBlur'` form now also keeps its errors, rather than clearing the ones for a changed field, until the field is left; previously the per-derivation validation pass hid that the clearing had never run for it ([#5306](https://github.com/rjsf-team/react-jsonschema-form/pull/5306))
...
- Fixed an `ErrorSchema` a field raises through `onChange`, on a path that already carried a validator error, being lost as soon as the form derived state again, so a controlled form flickered the error once per keystroke ([#5306](https://github.com/rjsf-team/react-jsonschema-form/pull/5306))
