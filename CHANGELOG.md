...
- Upgraded `@x0k/json-schema-merge` to `^1.0.5`, which now preserves Symbol-keyed properties (e.g. `Symbol(__rjsf_ref)`) when merging `allOf` schemas; removed the corresponding Symbol-preservation workaround from `retrieveSchemaInternal()`, fixing ([#5146](https://github.com/rjsf-team/react-jsonschema-form/issues/5146))
