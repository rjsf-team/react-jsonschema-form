...
- Collapsed each published package's `tsconfig.json` + `tsconfig.build.json` + `src/tsconfig.json` chain into one source config per package plus one test config extending a new shared `tsconfig.test.json`, so the options previously split across the root `tsconfig.base.json` and `tsconfig.build.json` and repeated in 15 test configs are each stated once. A source build now depends only on its dependencies' source builds — `tsc -b packages/core` no longer pulls `utils/test` or `validator-ajv8/test` into the graph
- Fixed the root `tsconfig.json` solution, which referenced `snapshot-tests` twice and omitted `mantine`, `primereact`, `validator-ata` and `validator-cfworker`. It now lists every source and test project exactly once, and a new root `typecheck` script (`tsc --build`) runs in CI. Test code was previously never typechecked outside the editor, which had left stale tests in `@rjsf/antd`, `@rjsf/chakra-ui`, `@rjsf/shadcn` and `@rjsf/validator-ata` plus two playground samples drifted from the current core API. Relatedly, `@rjsf/chakra-ui`'s `type-check` ran `tsc --noEmit` against a solution config with `files: []`, so it checked nothing at all; it now builds the source and test projects
- Every package now emits `lib/` with the same compiler settings. Before, only `@rjsf/core` emitted `esnext` with declaration maps while the rest emitted ES2018 with JS source maps, depending on which root config their `src/tsconfig.json` happened to extend. All of them now target `esnext` — every theme depends on core, so consumers already had to run `esnext` output — and ship both `.js.map` and `.d.ts.map`. `tsconfig.tsbuildinfo` is no longer written into `lib/` and published with it; in 6.8.0 it was 446 kB of `@rjsf/antd`'s 959 kB unpacked tarball and 265 kB of `@rjsf/mui`'s 856 kB
- Narrowed what library code can see: `"types": []` in `tsconfig.base.json`, so packages no longer pick up every hoisted `@types/*` package and the ones using Node APIs opt in explicitly, and `"lib": ["ESNext"]` in `@rjsf/validator-ajv8`, `-ata` and `-cfworker`, so a validator cannot compile against browser globals
- `build:ts` is now plain `tsc -b`. The `rimraf ./lib` it used to start with only guarded against stale outputs from renamed or deleted sources, which cannot reach a release because CI builds from a fresh checkout, and it forced deleting `tsconfig.tsbuildinfo` too (TypeScript trusts the build-info over the presence of outputs), so every build was a full rebuild. The build-info file is now an Nx `build` output alongside `lib/`, so a cache restore leaves the incremental state coherent
