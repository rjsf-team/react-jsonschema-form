...
- Fixed `liveValidate: 'onBlur'` validating whenever the form derived state from new props, rather than only when a field was left, so replacing the form data from outside showed the errors before any blur. Deriving state now also clears a changed field's errors for it, the way it does for live validation being off, and the clearing reaches the error list as well as the field, so the `ErrorList` and the `onChange` payload agree with what the field shows ([#5306](https://github.com/rjsf-team/react-jsonschema-form/pull/5306))
- Fixed live validation dropping every `extraErrors` entry when the form derived state from new props, so a server-supplied error disappeared as soon as a controlled parent replaced the form data ([#5306](https://github.com/rjsf-team/react-jsonschema-form/pull/5306))
- Fixed `extraErrors` and `customErrors` being merged into the errors a second time, the duplication half of [#4408](https://github.com/rjsf-team/react-jsonschema-form/issues/4408) (the clearing half was fixed by `getDerivedStateFromProps` in v6). It happened on three paths: deriving state under live validation without re-validating, so a re-render changing only `className` duplicated every server-supplied error; deriving state while the form's own data had diverged from the `formData` prop, which merged into the already-merged `errorSchema` and left the doubled result stored as the base for later derivations; and a field raising its own `ErrorSchema` through `onChange` with live validation off, which appended the extra errors to a list that already held them ([#5306](https://github.com/rjsf-team/react-jsonschema-form/pull/5306))
- Fixed a stored validation result outliving the schema that produced it under `liveValidate: 'onBlur'`, which no longer re-validates on a schema change: the next change to any other prop brought back an error the current schema cannot raise ([#5306](https://github.com/rjsf-team/react-jsonschema-form/pull/5306))
- Fixed an `ErrorSchema` a field raises through `onChange`, on a path that already carried a validator error, being lost as soon as the form derived state again for any other reason (a `className` change, a new `extraErrors` object); the raise now survives those derivations, though not a controlled parent echoing the change that raised it, which the changed-field clearing still treats as new data for that field. The stored error list carries the raise too, so the `ErrorList` says what the field does; a raise at the root, which replaces every displayed error, no longer replaces the stored validator result, which used to drop every other field's error for good; and a raise that hands back the displayed `errorSchema`, as the optional-data Add and Remove buttons and the fallback field's type change do, is stored without the `extraErrors` and `customErrors` it carries, which would otherwise stay after the parent cleared them, and a raise left with nothing but those leaves the validator's error standing; the item errors an `ArrayField` remaps after a reorder, remove or copy are displayed but not stored, since at their new indexes they no longer line up with the `extraErrors` to leave out. Under `liveValidate: 'onBlur'` a controlled form now clears a field's error when the parent hands back changed data for that field, whether a keystroke or an `ArrayField` reorder, remove or copy (which drops the item errors the array remapped), while an uncontrolled form keeps it until the field is left, as with live validation off ([#5306](https://github.com/rjsf-team/react-jsonschema-form/pull/5306))
