...
- Bumped `ata-validator` to `^0.18.2`. The 0.17 line read `safe-regex.js` through `fs.readFileSync(__dirname, ...)` at module init, which crashed the playground under Rolldown bundling. 0.18.2 defers the read to the first `bundleStandalone` call, so importing the validator in the browser no longer touches the filesystem; a `test_browser_nofs.js` regression test was added upstream. See [ata-core/ata-validator#25](https://github.com/ata-core/ata-validator/issues/25)
