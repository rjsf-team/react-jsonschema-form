...
- Fixed `memo()` widgets (and `forwardRef()` widgets on React 19) being rejected: `getWidget()` now checks for React's `$$typeof` component tag directly via the new exported `isWidgetComponent()` instead of using `react-is` (which only inspects elements), and `getUiOptions()` no longer mistakes a wrapper component passed as `ui:widget` for the deprecated `{component, options}` object form; this also adds `lazy()` widget support and removes the `react-is` dependency ([#5216](https://github.com/rjsf-team/react-jsonschema-form/pull/5216))
