...
- `build:ts` is now plain `tsc -b`. The `rimraf ./lib` it used to start with only guarded against stale outputs from renamed or deleted sources, which cannot occur in a release because CI builds from a fresh checkout, and it forced deleting `tsconfig.tsbuildinfo` too (TypeScript trusts the build-info over the presence of outputs), so every build was a full rebuild. The build-info file is now an Nx `build` output alongside `lib/`, so a cache restore leaves the incremental state coherent
