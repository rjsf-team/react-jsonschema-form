...
- Consolidated the TypeScript configuration: one `tsconfig.base.json` (plus a new `tsconfig.test.json` for the 15 near-identical test configs) now drives every package, replacing a root `tsconfig.build.json` whose options only ever reached `@rjsf/core`. Every package is now built at the same `ES2022` target and ships both source maps and declaration maps, where previously `@rjsf/core` emitted only declaration maps and every other package only source maps. Relative imports in `src` now carry explicit `.js` extensions, so `tsc` emits Node-loadable ESM directly and `tsc-alias` drops out of 13 of the 15 build scripts (`@rjsf/antd` and `@rjsf/validator-ajv8` keep it for their bare-specifier replacers). Also enabled `verbatimModuleSyntax`, completed the stale root project-reference list, and stopped `@rjsf/daisyui`'s malformed configs from compiling `src` in place
