...
- Fixed switching between `oneOf`/`anyOf` options leaving a property holding the option it was switched away from, so that switching back to an option restores its `default` rather than keeping the other option's. `MultiSchemaField` and `LayoutMultiSchemaField` now drop each value that still matches the default the old option would have produced before filling in the new option's defaults, which lets those defaults reach a key the old one had already populated. A value edited away from the old default no longer matches it and is kept, as is one for a property the new option declares without a `default` of its own. A value that still matches is replaced whoever put it there, including one supplied in `formData` that happens to equal the old option's default, since the two cannot be told apart, fixing [#4476](https://github.com/rjsf-team/react-jsonschema-form/issues/4476)
