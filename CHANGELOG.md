...
- Fixed `sanitizeDataForNewSchema()` leaving an `object` or `array` property holding the old subschema's `default` when switching between `oneOf`/`anyOf` options. The comparison that replaces a stale value only ran for scalar properties; object and array ones were recursed into instead, so a nested object, array or array of objects kept the previous option's default. Each value is now measured against the default the form actually holds for it — the one `getDefaultFormState()` computes for the whole object, with the children's own defaults merged in and the undeclared keys of a `default` dropped — so that detection and replacement agree and switching back to an option restores its default. A default sitting on an array's `items` is applied per element, whether the items are objects or scalars, and a `default` the object around a property spells out for it counts as that property's default too, fixing [#4476](https://github.com/rjsf-team/react-jsonschema-form/issues/4476)
- Changed `sanitizeDataForNewSchema()` to leave a value alone when the constraint it is being measured against did not change between the two schemas, so a property the option switch never touched keeps its data: a `readOnly` field holding a server-supplied value that differs from its schema `default` is no longer cleared on every switch, and an `object` or `array` value is no longer cleared or substituted by a `const` or enum-like constraint that neither schema changed. A scalar's `const` and `enum` are still applied unconditionally, since a dependency can narrow them without the property's own schema changing, and a scalar the new schema newly marks `readOnly` is still cleared so its default takes over
- Changed `sanitizeDataForNewSchema()` to stop clearing an `object` or `array` value held by a `readOnly` field whose `default` differs between the two schemas; such a value is recursed into, since clearing it left the field holding neither the server's data nor the new default. A `const` the new schema puts on an `object` or `array` property is now written to it instead of clearing it, and an enum-like check is skipped altogether for a value whose `oneOf`/`anyOf` carries an option that is not `const`-like, since the values collected from the remaining options do not describe everything the schema allows
- Changed `sanitizeDataForNewSchema()` to disregard `undefined`-valued keys when comparing a value against a default or `const`, since this function writes such keys itself when clearing data. The comparison is a `fast-equals` comparator rather than a copy of each operand, so it allocates nothing and tolerates a self-referential value the way `deepEquals()` does
- Changed `sanitizeDataForNewSchema()` to copy a replacement value before putting it in the form data, so that no two properties or array elements share one object and mutating the returned data cannot reach back into a `default`, `const` or `enum` of a schema that other forms and renders are still using
