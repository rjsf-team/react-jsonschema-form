...
- Changed `getInputProps()` to default `number`/`integer` schemas to a `text` input with `inputMode="decimal"`/`"numeric"` and a digits-only `pattern` instead of a native `<input type="number">`, for the themes that render a plain native `<input>` (an explicit `ui:options.inputType` still opts back into a native `number`/other input). A native number input inconsistently filters keystrokes across browsers, silently discards values it can't parse, ignores mouse-wheel scrolling over a focused field, and never accepts a non-`.` locale decimal separator; a text input with a numeric `inputMode` gets the same numeric keyboard on mobile without those problems, and the `pattern` still lets the browser's native constraint validation reject a non-numeric value the same way it already does for `required`, fixing [#4038](https://github.com/rjsf-team/react-jsonschema-form/issues/4038)
