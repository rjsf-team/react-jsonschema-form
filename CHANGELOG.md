...
- Removed the `jsonpointer` dependency; `findSchemaDefinition()` now resolves JSON pointer fragments through `getByPath()`, so a `#/__proto__` ref finds nothing instead of `Object.prototype`
