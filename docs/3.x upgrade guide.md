...
## noHtml5Validate now defaults to true

In v3.x, `noHtml5Validate` defaults to true. Seem more about [this change here](https://github.com/rjsf-team/react-jsonschema-form/issues/1184).

If you were previously using HTML5 validatins, and want to continue doing so, you can reenable that behavior by setting the prop `noHtml5Validate=false` on your `Form` elements. 