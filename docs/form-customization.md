...
> Note that storing large dataURIs into form state might slow rendering, preferably use `blob-url` for faster rendering.
...
Traditional `data-url` approach is much slower for modern browser, so `blob-url` will create `Blob` exposed URL for local files, and further can be used to upload.
[blob-url](https://developer.mozilla.org/en-US/docs/Web/API/Blob#Example_for_creating_a_URL_to_a_typed_array_using_a_blob)s.

There are two ways to use blob file widgets.

1. By declaring a `string` json schema type along a `blob-url` [format](#string-formats):
...
2. By specifying a `ui:widget` field uiSchema directive as `blobFile`:
...
Multiple files selectors are supported by defining an array of strings having `blob-url` as a format:
...
> Note: To upload blob files, use `let blob = await fetch(url).then(r => r.blob());`
