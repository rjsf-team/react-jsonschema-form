...
>(
  ...
  if (pathList.length) {
    ...
    pathList.forEach((subPath) => {
      ...
      if (hasByPath(parentField, ANY_OF_KEY)) {
        // if this sub-path has an `anyOf` then use the formData to drill into the schema with the selected option
      ...
      } else if (hasByPath(parentField, ONE_OF_KEY)) {
