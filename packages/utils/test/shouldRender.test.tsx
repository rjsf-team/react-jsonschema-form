...
type MyComponentProps = {
  myProp: number | VoidFunc | {
    mySubProp: number | VoidFunc;
  }
};

type MyComponentState = {
  myState: number | VoidFunc | {
...
describe('shouldRender()', () => {
  describe('single level comparison checks', () => {
    ...
    it('should handle equivalent function prop', () => {
      const fn = () => {};
      initial = { props: { myProp: fn }, state: { myState: 1 } } as React.Component<MyComponentProps, MyComponentState>;
      expect(
        shouldRender(initial, { myProp: fn }, { myState: 1 })
      ).toBe(false);
    });
  });
