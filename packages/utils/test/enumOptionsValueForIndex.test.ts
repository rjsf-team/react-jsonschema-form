...
describe('enumOptionsValueForIndex()', () => {
  ...
  it('returns undefined for a single index >= array size', () => {
    expect(
      enumOptionsValueForIndex(ALL_OPTIONS.length, ALL_OPTIONS)
    ).toBeUndefined();
  });
  it('returns emptyValue for a single negative index', () => {
    expect(enumOptionsValueForIndex('-1', ALL_OPTIONS, EMPTY_VALUE)).toBe(
      EMPTY_VALUE
    );
  });
  it('returns emptyValue for an empty string', () => {
    expect(enumOptionsValueForIndex('', ALL_OPTIONS, EMPTY_VALUE)).toBe(
      EMPTY_VALUE
    );
  });
  it('returns emptyValue for a single index >= array size', () => {
    expect(
      enumOptionsValueForIndex(ALL_OPTIONS.length, ALL_OPTIONS, EMPTY_VALUE)
    ).toBe(EMPTY_VALUE);
  });
  ...
  it('returns value of option at a valid index', () => {
    expect(enumOptionsValueForIndex(2, ALL_OPTIONS)).toEqual(
      ALL_OPTIONS[2].value
    );
  });
  it('returns empty array for a list of bad indexes', () => {
    expect(
      enumOptionsValueForIndex(['-2', ALL_OPTIONS.length + 1], ALL_OPTIONS)
    ).toEqual([]);
  });
  it('returns empty array for a list of bad indexes, even with empty value', () => {
    expect(
      enumOptionsValueForIndex(
        ['-2', ALL_OPTIONS.length + 1],
        ALL_OPTIONS,
        EMPTY_VALUE
      )
    ).toEqual([]);
