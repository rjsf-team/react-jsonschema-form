...
describe('getDisplayLabel()', () => {
  ...
  it('with ui:field', () => {
    ...
    expect(schemaUtils.getDisplayLabel(schema, { 'ui:field': 'test' })).toEqual(
      false
    );
  });
  describe('array type', () => {
    it('items', () => {
      expect(
        getDisplayLabel(testValidator, { type: 'array', items: { type: 'string' } }, {})
      ).toEqual(false);
    });
    it('items enum', () => {
      expect(
        getDisplayLabel(testValidator, { type: 'array', enum: ['NW', 'NE', 'SW', 'SE'] }, {})
      ).toEqual(false);
    });
    it('files type', () => {
      expect(
        getDisplayLabel(testValidator, { type: 'array' }, { 'ui:widget': 'files' })
      ).toEqual(true);
    });
    it('custom type', () => {
      expect(
        getDisplayLabel(testValidator,
