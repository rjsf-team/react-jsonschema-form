...
describe("enumOptionsDeselectValue()", () => {
  ...
  it("returns same selection when index > length", () => {
    selected = enumOptionsDeselectValue(
      ALL_OPTIONS.length,
      ALL_VALUES,
      ALL_OPTIONS
    );
  ...
  it("removes a third value from a selected list", () => {
    expect(enumOptionsDeselectValue(2, selected, ALL_OPTIONS)).toEqual([
      ALL_VALUES[3],
    ]);
  });
  it("returns the selected value when index does not match value", () => {
    expect(enumOptionsDeselectValue(1, ALL_VALUES[0], ALL_OPTIONS)).toBe(
      ALL_VALUES[0]
    );
  });
  it("returns undefined when index does not matches value", () => {
    expect(
      enumOptionsDeselectValue("0", ALL_VALUES[0], ALL_OPTIONS)
    ).toBeUndefined();
  });
  it("returns undefined when index does not matches value", () => {
