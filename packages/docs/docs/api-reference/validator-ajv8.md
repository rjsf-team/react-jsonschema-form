...
- [options={}]: CustomValidatorOptionsType - The optional map of `CustomValidatorOptionsType` options that are used to create the `ValidatorType` instance. Supported options:
  ...
  - `suppressDuplicateFiltering`: Controls filtering of duplicate `anyOf`/`oneOf` errors. `'anyOf'` suppresses filtering for `anyOf` (oneOf duplicates still filtered), `'oneOf'` suppresses filtering for `oneOf` (anyOf duplicates still filtered), `'all'` disables all duplicate filtering. When omitted, duplicates are filtered for both (default behavior).
...
- [localizer]: Localizer | undefined - If provided, is used to localize a list of Ajv `ErrorObject`s after running the form validation using AJV
- [suppressDuplicateFiltering]: `'anyOf' | 'oneOf' | 'all' | undefined` - Controls filtering of duplicate `anyOf`/`oneOf` errors. See `customizeValidator()` for full description of each value.
