...
- Precompiled validators (`compileSchemaValidators` / `createPrecompiledValidator`) are supported. `ata-validator`'s `bundleStandalone` output is adapted into the validator function map the precompiled consumer expects. Two constraints carry over from the standalone (AOT) path: custom formats must be a RegExp or pre-anchored string pattern, since a function checker cannot be serialized into the bundle and is rejected at compile time; and an invalid `anyOf` or schema-valued `additionalProperties` reports a single error on the parent field rather than per-field errors. Validity is unaffected in both cases.
