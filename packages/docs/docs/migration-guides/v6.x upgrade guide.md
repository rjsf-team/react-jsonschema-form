...
During the conversion of `ObjectField` to a stateless functional component, the `additionalProperties`-based props in these two interfaces were simplified by replacing two callback-generators with a memoized callbacks.
...
The `onKeyChange: (value: string) => () => void` callback-generator function was replaced with a simpler, memoizable, callback, along with a helper callback (since most of the key renames were triggered off of an input blur):
...
The `onDropPropertyClick: (value: string) => () => void` callback-generator function was replaced with a simpler, memoizable, callback:
...
The `onAddClick: (schema: S) => () => void` callback-generator function was replaced with a simpler, memoizable, callback:
...
In addition to this refactor, the conversion of the `ArrayField` template to a functional component caused the replacement of the 4 callback-generator functions with 5 memoizable functions.

##### onAddIndexClick replacement

The `onAddIndexClick: (index) => (event?) => void` callback-generator function was replaced with a simpler, memoizable, callback:
...
The `onCopyIndexClick: (index: number) => (event?: any) => void;` callback-generator function was replaced with a simpler, memoizable, callback:
...
The `onDropIndexClick: (index) => (event?) => void` callback-generator function was replaced with a simpler, memoizable, callback:
...
The `onReorderClick: (index, newIndex) => (event?) => void` callback-generator function was replaced with 2 simpler, memoizable, callbacks:
...
In order to support the conversion of `ArrayField` to a functional component, the `items` property was changed from an
array of `ArrayFieldItemTemplateType` information to an array of React elements. These elements are result of the `ArrayField`
actually rendering the `ArrayFieldItemTemplate` directly, rather than deferring it the `ArrayFieldTemplate`.

If you previously simply provided a custom `ArrayFieldTemplate` to do the full rendering of your arrays, now you will have to
...
> NOTE: the example above could also have implemented an `ArrayFieldItemButtonsTemplate` and used that as well.
