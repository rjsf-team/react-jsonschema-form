...
export type MantinePropsTypes<T = any, S extends StrictRJSFSchema = RJSFSchema, F extends FormContextType = any> = {
  formContext?: F;
...
export type MantineErrorPropsType<T = any, S extends StrictRJSFSchema = RJSFSchema, F extends FormContextType = any> = {
  formContext?: F;
  uiSchema?: UiSchema<T, S, F>;
  options?: UIOptionsType<T, S, F>;
...
/**
 * Extract props meant for mantine UI components from props that are
 ...
 * @returns {any}
 */
export function getMantineProps<T = any, S extends StrictRJSFSchema = RJSFSchema, F extends FormContextType = any>({
  formContext = {} as F,
  uiSchema = {},
  options = {},
  defaultSchemaProps = { fluid: true, inverted: false },
  defaultContextProps = {},
}: MantinePropsTypes<T, S, F>) {
  const formContextProps = formContext.mantine;
  const schemaProps = getUiOptions<T, S, F>(uiSchema).mantine;
  const optionProps = options.mantine;
...
/**
 * Extract error props meant for mantine UI components from props that are
 * passed to Widgets, Templates and Fields.
 * @param {Object} params
 * @param {Object?} params.formContext
 * @param {Object?} params.uiSchema
 ...
 * @returns {any}
 */
export function getMantineErrorProps<
  T = any,
...
}: MantineErrorPropsType<T, S, F>) {
  const formContextProps = formContext.mantine && formContext.mantine.errorOptions;
  const mantineOptions: GenericObjectType = getUiOptions<T, S, F>(uiSchema).mantine as GenericObjectType;
  const schemaProps = mantineOptions && mantineOptions.errorOptions;
  const optionProps = options.mantine && (options.mantine as GenericObjectType).errorOptions;
