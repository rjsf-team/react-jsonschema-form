...
- `Form` now passes its `uiSchema` through to `getDefaultFormState()` so `ui:emptyValue` and `ui:initialValue` are applied, and folds `ui:required: true` fields into the schema's `required` before validating (via `@rjsf/utils`'s new `augmentSchemaWithUiRequired()`) so schema validation sees them too. `SchemaField` and `LayoutGridField` resolve a field's effective required state from `ui:required` when present, falling back to the schema otherwise ([#4983](https://github.com/rjsf-team/react-jsonschema-form/issues/4983))
...
- **BREAKING CHANGE** Extended `ui:emptyValue` to apply whenever a field is blank — on initial render and after a form reset, not just when a user clears a widget — by threading `uiSchema` through the `getDefaultFormState`/`computeDefaults`/`getObjectDefaults` pipeline (`getDefaultFormState()` and `SchemaUtilsType.getDefaultFormState()` gained a trailing `uiSchema` parameter). Added `ui:initialValue`, which pre-fills a field on render/reset and takes priority over `schema.default`, and `ui:required`, which overrides a field's `required` status from the UI side only (`true` adds the required indicator and folds the field into schema validation via the new `augmentSchemaWithUiRequired()` export; `false` hides the indicator without suppressing schema-level validation, warning via `console.warn` when used on a schema-required field with neither `ui:initialValue` nor `ui:emptyValue` set) ([#4983](https://github.com/rjsf-team/react-jsonschema-form/issues/4983))
