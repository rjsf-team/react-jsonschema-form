...
- Bumped `react`/`react-dom`/`@types/react`/`@types/react-dom` further, to 19.3.0. Re-added a `pnpm-workspace.yaml` override pinning `react`/`react-dom` to a single version, which had briefly been removed as redundant once everything in the repo required `>=19`: at 19.3.0 that no longer held — without the override, `pnpm install` forked the graph into two react copies (19.2.8 and 19.3.0) with `react-dom@19.3.0` paired against `react@19.2.8` in part of the tree, breaking hooks across that boundary (confirmed via `pnpm why react` and a real test failure inside `@rjsf/shadcn`'s `fancy-multi-select.tsx`)
