...
- **BREAKING CHANGE** Every package is now ESM-only: the CommonJS, bundled ESM and UMD builds in `dist/` and the `./dist` and `./dist/*.cjs` export paths are gone; `lib/` (per-file ESM plus declarations) is the only published output. `require()` of the packages keeps working on every supported Node version via native `require(esm)`
- Replaced the per-package `tsc -b` emit plus `esbuild` + `rollup` bundling with a single `tsdown` run driven by one shared `tsdown.base.mts`, which emits `lib/`. The build only transpiles; typechecking is the separate root `tsc --build`, and tsc no longer emits JavaScript (the package tsconfigs are `emitDeclarationOnly`)
